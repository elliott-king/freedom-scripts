...
def events(table=dyn_upload.DEV_EVENTS_TABLE):
    events = []
    ddb_names = set(
        [x["name"].lower() for x in dyn_upload.get_all_items_from_table(table)]
    )
    scraped_names = set()
    for suffix in [""] + [nextpage + str(x) for x in range(2, 10)]:
        ...
        for d in divs:
            ...
            if (
                "rsvp" in info["description"].lower()
                or "regist" in info["description"].lower()
            ):
                info["rsvp"] = True
            ...
            types.add_type(
                info,
                "athletics",
                ["athletics", "exercise", "fitness", "physique", "aerobics"],
            )
            types.add_type(
                info,
                "crafts",
                ["craft", "coloring", "knitting", "sculpt", "creative writing"],
            )
            ...
            if (
                info["name"].lower() not in scraped_names
                and info["name"].lower() not in ddb_names
            ):
